 ...
 * PROBLEM STATEMENTS
Codes and Ciphers
Codes and Ciphers have been used to protect secrets since ancient times. In 1882, Frank Miller described the use of a one-time pad: an encryption technique that cannot be cracked if used correctly. The technique involves the use of a key and plaintext that is to be coded. Each character of the plaintext and the key are paired up using modular addition.
...
public class Cipher {
	...
	public String encrypt(String input) {
		...
		for (int ch : modArray) {
			...
			CipherText += alphabets.get(ch - 1);
		}
		CipherText = capitalize(CipherText);
		if (DEBUG)
			System.out.println();
		if (DEBUG)
	...
	public String decrypt(String CipherText) {
		...
		for (int i = 0; i < size; i++) {
			...
			char ch = CipherText.toCharArray()[i];
	...
	public static String replaceEndSpecialChar(String myString) {
		...
		if (myString.substring(myString.length() - 1).equals("!")
				|| myString.substring(myString.length() - 1).equals(".")) {
	...
	public static boolean checkValidInput(String myString) {
		...
		/*
		 * if(myString.substring(myString.length()-1).equals("!")){ myString
